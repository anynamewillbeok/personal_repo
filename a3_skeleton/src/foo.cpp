                       ...
                       std::shared_ptr<std_srvs::srv::Trigger::Response> res)
{
    ...
    RCLCPP_INFO_STREAM(this->get_logger(),"service valid segments:" << segments);
    res->success = true;
    res->message = "Segments: " + std::to_string(segments);
}
...
void Foo::threadFunction()
{
    ...
    while(rclcpp::ok()){
        //This function will run every second
         if(laserProcessingPtr_ != nullptr){
            unsigned int segments = laserProcessingPtr_->countSegments(); // segments here is a local variable, so not dealing with threading
            RCLCPP_INFO_STREAM(this->get_logger(),"In thread valid segments:" << segments);
         }
